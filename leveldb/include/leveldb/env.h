...
// preprocessor direcives. run before compiler.
// #define #ifndef #endif 除非include guard其他不建议用
...
class LEVELDB_EXPORT Env {
 public:
  Env();//non-default constructor

  Env(const Env&) = delete;//copy constructor
  Env& operator=(const Env&) = delete; //assignment constructor

  virtual ~Env();
// virtual:subclass overriding
// overrding without virtual; polymorphism
  ...
  static Env* Default();//a pointer to a heap-allocated evn implementation
  // mac linux都是env_posix系统，但是windows是env_windows.cc
  // db::open()
...
//  在接口类无法得知如何定义，所以要mark成pure virtual function，表示这个系统允许出现多个，但是再db也是，但是只有1个，所以直接实例化return出来
  ...
  virtual Status NewAppendableFile(const std::string& fname,
                                   WritableFile** result);
// table cache. ->SST ->file handler; *result 在系统层面已经打开了，就没必要重复打开
...
class LEVELDB_EXPORT RandomAccessFile {
 public:
  ...
  /*
  scratch 为了释放内存用的
  RAII->constructor, deconstructor
  */
};
...
class LEVELDB_EXPORT Logger {
 public:
  Logger() = default;
  // 对virtual type，无法实例化，它的constructor不会直接调用，之所以需要设为default，因为subclass会调用superclass的constructor，
  // 除非子类explicit指明了
...
// prepro directive -> conditional compilation
...
// 这里Env*env，没有加constant，那就说明要么env要变化，要么要调用env的non-constant函数，因为要继承父类的类型，
// 或者是多个线程不能同时调用
LEVELDB_EXPORT Status WriteStringToFile(Env* env, const Slice& data,
                                        const std::string& fname);
 // 为什么不是pass by value,因为string可能比较大，就pass by reference
...
class Student{
  ...
  private:
    ...
    double height;
    std :: string name;//class type that includes a default constructor
    Pet pet;//unknown class type
};

Student stu; //default initialization, uncertain. local variable
Student arr[5]; //value initialization, zeroed. array.

sythetsize constructor ~= user-defined empty constructor with empty initialized list
1. class type that includes default constructor(not including synthetised default constructor)
2. class that not including default constructor. Recursively initialize each one of its member either by value or default initialization
3. primitive/built-in type member -> default: unknown value; value:zero


explicit把所有的copy initialization disabled了
*/